...
public class AESUtil {
    ...
    public static byte[] encryptAndDecrypt(byte[] buffer,String appsecret,Integer mode) throws Exception{
        ...
        //  微信退款回调接口说明： 用key*对加密串B做AES-256-ECB解密（PKCS7Padding）
        //  微信退款接口：  https://pay.weixin.qq.com/wiki/doc/api/native.php?chapter=9_16&index=11
    ...
    public static void main(String[] args) throws Exception{
        ...
        //加密
        byte[] bytes = encryptAndDecrypt(txt.getBytes("UTF-8"), appsecret, mode);
        // 直接 new String(bytes) 返回的是乱码的，所以考虑用  Base64Util 加密 解密
        String encode = Base64Util.encode(bytes);
        System.out.println(encode);
